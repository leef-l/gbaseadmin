...
func (g *Generator) ensureDirectory(db *sql.DB, appName, path string) (int64, error) {
	var id int64
	err := db.QueryRow("SELECT id FROM system_menu WHERE path = ? AND type = 1 AND deleted_at IS NULL", path).Scan(&id)
	if err == nil {
...
func (g *Generator) ensureMenu(db *sql.DB, parentID int64, title, path, component, permission string, sort int, isShow int) (int64, bool, error) {
	var id int64
	err := db.QueryRow("SELECT id FROM system_menu WHERE path = ? AND type = 2 AND deleted_at IS NULL", path).Scan(&id)
	if err == nil {
		if g.config.Force {
			_, err = db.Exec(
...
func (g *Generator) ensureButton(db *sql.DB, parentID int64, title, permission string, sort int) (bool, error) {
	var id int64
	err := db.QueryRow("SELECT id FROM system_menu WHERE permission = ? AND type = 3 AND deleted_at IS NULL", permission).Scan(&id)
	if err == nil {
		if g.config.Force {
			_, err = db.Exec(`UPDATE system_menu SET title=?, sort=?, updated_at=NOW() WHERE id=?`, title, sort, id)
			if err != nil {
