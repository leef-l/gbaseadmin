...
使用说明:
  ...
  2. 运行测试:
     python bug整理/e2e_test.py

  3. 输出:
     - bug整理/bug列表.md    — 发现的 Bug 列表（每次运行重新生成）
     - bug整理/screenshots/  — 测试过程截图

测试覆盖:
  后台管理端:
    - 登录（用户名密码 + 滑块验证码拖动）
    - 菜单遍历（展开所有一级菜单，逐个点击叶子菜单，检查 404/500）
    - CRUD 操作（用户/角色/部门/菜单 的新增和编辑弹窗）

  WAP 端:
    - 手机号验证码登录（验证码固定 123456）
    - 11 个页面遍历（首页/分类/活动/个人中心/钱包/余额/领券/优惠券/订单/设置/我的）
    - 用户功能子页面（充值/领券/优惠券/余额/评价/设置）
    - 活动完整流程（参与→填写→完成步骤→领取奖励→验证钱包）
    - 领券功能
    - 订单 tab 切换
