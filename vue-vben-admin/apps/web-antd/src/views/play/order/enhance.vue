...
const statusActions: Record<number, { label: string; status: number; danger?: boolean }[]> = {
  0: [{ label: '确认支付', status: 1 }, { label: '取消', status: 4, danger: true }],
  1: [{ label: '开始服务', status: 2 }, { label: '取消', status: 4, danger: true }],
  2: [{ label: '完成', status: 3 }],
  3: [{ label: '退款', status: 5, danger: true }],
  5: [{ label: '确认退款', status: 6 }],
};

/** 变更订单状态 */
function handleChangeStatus(row: OrderItem, targetStatus: number, label: string) {
  Modal.confirm({
    title: `${label}确认`,
    content: `确定要将订单 ${row.orderNo} ${label}吗？`,
    okType: targetStatus === 4 || targetStatus === 5 ? 'danger' : 'primary',
    async onOk() {
    ...
    <Grid>
      ...
      <template #action="{ row }">
        <template v-for="act in (statusActions[row.orderStatus] || [])" :key="act.status">
          <Button type="link" :danger="act.danger" size="small" @click="handleChangeStatus(row, act.status, act.label)">{{ act.label }}</Button>
        </template>
