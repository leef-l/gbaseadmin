...
const gridOptions: VxeGridProps<PaymentItem> = {
  columns: [
    ...
    { field: 'payAmount', title: '支付金额（元）', formatter: ({ cellValue }: { cellValue: number | null }) => cellValue != null ? (cellValue / 100).toFixed(2) : '-' },
    { field: 'payStatus', title: '支付状态', width: 120, slots: { default: 'payStatus_cell' } },
    { field: 'refundAmount', title: '退款金额（元）', formatter: ({ cellValue }: { cellValue: number | null }) => cellValue != null ? (cellValue / 100).toFixed(2) : '-' },
