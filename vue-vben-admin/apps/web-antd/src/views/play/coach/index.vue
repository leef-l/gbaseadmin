...
const gridOptions: VxeGridProps<CoachItem> = {
  columns: [
    ...
    { field: 'incomeTotal', title: '累计收入（元）', formatter: ({ cellValue }: { cellValue: number | null }) => cellValue != null ? (cellValue / 100).toFixed(2) : '-' },
    { field: 'incomeBalance', title: '可提现余额（元）', formatter: ({ cellValue }: { cellValue: number | null }) => cellValue != null ? (cellValue / 100).toFixed(2) : '-' },
